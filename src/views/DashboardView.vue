<template>
  <div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div v-else>
      <!-- Trophées -->
      <div v-if="excellentTests.length > 0" class="mb-8">
        <h2 class="text-lg font-bold text-republic-blue-800 mb-3 flex items-center gap-2">
          <span class="text-2xl">🏆</span> Trophées d'excellence
        </h2>
        <div class="flex flex-wrap gap-3">
          <div
            v-for="t in excellentTests"
            :key="t.id"
            class="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-2xl bg-gradient-to-r from-republic-gold-400 to-republic-gold-500 text-white shadow-lg shadow-republic-gold-300/40"
          >
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
            </svg>
            <div>
              <p class="text-xs font-bold opacity-80">Excellence — {{ t.examTypeName }}</p>
              <p class="text-sm font-extrabold">{{ t.score }}/{{ t.total }} · {{ t.percentage }}%</p>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <!-- Excellence badge -->
                <span
                  v-if="isExcellent(test)"
                  class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-gradient-to-r from-republic-gold-400 to-republic-gold-500 text-white"
                >
              ...
              <p class="text-xs text-gray-400 mt-1">
                ...
                <span v-if="test.timeSpent">
                  · {{ test.percentage >= passThreshold ? 'Reussi en' : 'Termine en' }} {{ formatTimeSpent(test.timeSpent) }}
                </span>
              </p>
            </div>
...
const excellentTests = computed(() =>
  testHistory.value.filter(t => isExcellent(t))
)

function isExcellent(test) {
  // Badge: CR or NAT, score >= 38/40
  return ['cr', 'nat'].includes(test.examTypeId) && test.score >= 38 && test.total >= 40
}
